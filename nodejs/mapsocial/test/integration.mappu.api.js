...
describe('Mappu API', function () {
    ...
    it('GET /social/tags?layer=topp:states&tags=a,b should return data', function (done) {
        http.request()
            .get('/social/tags?layer=topp:states&tags=a,b&bbox=0,0,1000.00,1000.00')
            .expect(new RegExp('\\{"content":\\[\\{"id":[0-9]*,"tags":"a,b,c","x":1.5,"y":2.4\\}\\]\\}'), done);
    });

    it('GET /social/tags?layer=topp:notexists&tags=a,b should return no data there is no such layer', function (done) {
        http.request()
            .get('/social/tags?layer=topp:notexists&tags=a,b&bbox=0,0,1000.00,1000.00')
            .expect('{"content":[]}', done);
    });
});
